...
let rec eval1 t = function
    ...
    | TmPred (fi, TmZero) -> TmZero (fi)
    | TmPred (fi, TmSucc (_, t1)) when isnumericval t1 ->
        t1
    | TmPred (fi, t1) ->
        let t1' = eval1 t1 in TmPred (fi, t1')
    | TmIsZero (fi, TmZero _) -> TmTrue
    | TmIsZero (_, TmSucc (_, t1)) when isnumericval t1 -> TmFalse
    | TmIsZero (fi, t1) ->
